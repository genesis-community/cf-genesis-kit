...
use Genesis qw/info warning error bail new_enough/;
use JSON::PP;
...
sub _perform_feature_pre_validation {
	...
	for my $want (@{$self->{raw_features}}) {
      ...
      push @curated_features, "+internal-db" unless $self->want_feature_in_list("+internal-db", \@curated_features);
			...
			warning("The #c{azure} feature does not have to be specified, as it will automatically be applied when deploying via an Azure CPI");
			...
			bail("The #c{local-ha-db} feature is no longer able to be supported.\n\tConsider using external database for high-availability.");
		} elsif ($want =~ /^(autoscaler|autoscaler-postgres)$/) {
			bail("The #c{$want} feature is no longer embedded in the #c{cf} kit.\n\tPlease see the cf-app-autoscaler genesis kit.");
		} elsif ($want eq "native-garden-runc") {
			warning("The #c{$want} feature is no longer supported; it is replaced by the upstream\n\t#c{cf-deployment/operations/experimental/use-native-garden-runc-runner} feature.");
			push @curated_features, "cf-deployment/operations/experimental/use-native-garden-runc-runner";
		} elsif ($want =~ /^(app-bosh-dns|dns-service-discovery)$/) {
			warning("The #c{$want} feature is no longer supported; it has been replaced by the\n\tupstream #c{cf-deployment/operations/enable-service-discovery} feature.");
			...
			push @curated_features, "enable-service-discovery" unless $self->want_feature_in_list("bare", \@curated_features);
		} elsif ($want eq "compiled-releases") {
			push @curated_features, "compiled-releases" unless $self->want_feature_in_list("cf-deployment/operations/use-compiled-releases", \@curated_features);
		} elsif ($want =~ /^(small-footprint|cf-deployment\/operations\/scale-to-one-az)$/) {
			push @curated_features, "small-footprint" unless $self->want_feature_in_list("small-footprint", \@curated_features);
		} elsif ($want =~ /^(nfs-volume-services|cf-deployment\/operations\/enable-nfs-volume-service)$/) { # Corrected regex
			push @curated_features, "nfs-volume-services" unless $self->want_feature_in_list("nfs-volume-services", \@curated_features);
		} elsif ($want =~ /^(smb-volume-services|cf-deployment\/operations\/enable-smb-volume-service)$/) { # Corrected regex
			push @curated_features, "smb-volume-services" unless $self->want_feature_in_list("smb-volume-services", \@curated_features);
		} elsif ($want =~ /^(nfs-ldap|nfs-ldap-tls|cf-deployment\/operations\/enable-nfs-ldap)$/) { # Corrected regex
			if (!$self->want_feature_in_list('nfs-volume-services', \@curated_features) && !$self->want_feature_in_list("cf-deployment/operations/enable-nfs-volume-service", \@curated_features)) {
				bail("Feature #c{$want} cannot be specified without feature #c{nfs-volume-services}");
			}
			push @curated_features, $want unless $self->want_feature_in_list($want, \@curated_features);
			...
			if ($self->want_feature_in_list("ocfp", \@curated_features)) { # check against already curated ocfp
				bail("Cannot specify blobstore with ocfp feature. \n\tWith ocfp feature blobstore specifies you.");
			}
			push @curated_features, $want unless $self->want_feature_in_list($want, \@curated_features);
			...
			if ($want eq "enable-service-discovery" && $self->want_feature_in_list("bare", \@curated_features)) {
				# do nothing if bare
			} else {
				push @curated_features, $want unless $self->want_feature_in_list($want, \@curated_features);
			}
			...
			if (-f "$want.yml") {
				...
				bail("#c{$want} was not found in upstream files.\n\tSee cf-deployment for valid ops files.");
			}
		} elsif ($want eq "ocfp") {
			push @curated_features, "enable-service-discovery" unless $self->want_feature_in_list("enable-service-discovery", \@curated_features) || $self->want_feature_in_list("bare", \@curated_features);
			push @curated_features, "uaa-admin-client" unless $self->want_feature_in_list("uaa-admin-client", \@curated_features);
			push @curated_features, $want unless $self->want_feature_in_list($want, \@curated_features);
		} else {
	...
	if ($self->want_feature_in_list("ocfp", \@curated_features)) {
		...
		if ($blobstore_feature_for_ocfp && !$self->want_feature_in_list($blobstore_feature_for_ocfp, \@curated_features)) {
			...
			for my $f (@curated_features) {
				if ($f =~ /^(minio-blobstore|aws-blobstore|aws-blobstore-iam|azure-blobstore|gcp-blobstore)$/ && $f ne $blobstore_feature_for_ocfp) {
	...
	if (!$self->{db_specified_flag} && !$self->want_feature_in_list('bare', \@curated_features)) {
		my $has_db = 0;
...
sub perform {
	...
	if (!$self->want_feature("bare")) {
		...
		if ($self->{iaas_name} eq 'azure' || $self->want_feature("small-footprint") ||
			...
			unless ($self->want_feature("small-footprint")){ # Avoid double-adding if small-footprint adds it
	...
	if ($self->want_feature("+migrated-v1-env") ||
		$self->want_feature("azure-blobstore") || $self->want_feature('minio-blobstore') ||
		$self->want_feature('aws-blobstore') || $self->want_feature('gcp-blobstore')) {
		if ($self->want_feature('bare')) {
			bail("Cannot have #C{bare} feature when migrating from v1 or using these blobstores in v1 migration context.");
		}
	...
	for my $feature (@{$self->{processed_features}}) {
		...
		if ($feature eq "azure-blobstore") {
			...
			$self->add_files(
				"overlay/blobstore/external.yml", "overlay/blobstore/azure.yml",
			...
			push @{$self->{blobstore_selections}}, $feature;
			$self->add_files("overlay/blobstore/external.yml", "overlay/blobstore/aws.yml", "cf-deployment/operations/use-external-blobstore.yml");
			if ($feature eq "aws-blobstore-iam" || $self->want_feature("aws-blobstore-iam")) { # check both original and potentially curated
				$self->add_files("overlay/blobstore/aws-iam.yml");
			}
		} elsif ($feature eq "minio-blobstore") {
			push @{$self->{blobstore_selections}}, $feature;
			$self->add_files("overlay/blobstore/external.yml", "overlay/blobstore/minio.yml", "cf-deployment/operations/use-external-blobstore.yml");
		} elsif ($feature eq "gcp-blobstore") {
			push @{$self->{blobstore_selections}}, $feature;
			if ($self->want_feature("gcp-use-access-key")) {
				$self->add_files("overlay/blobstore/external.yml", "cf-deployment/operations/use-external-blobstore.yml", "cf-deployment/operations/use-gcs-blobstore-access-key.yml");
			} else {
		...
		# Databases
			...
			if ($self->want_feature('+override-db-names')) {
				$self->add_files("operations/db-override-names.yml", "operations/db-override-postgres-names.yml", "overlay/db/internal-overrides.yml");
				if ($self->want_feature('+migrated-v1-env')) {
			...
			if ($self->want_feature('+override-db-names')) {
				$self->add_files("operations/db-override-names.yml", "operations/db-override-mysql-names.yml", "overlay/db/internal-overrides.yml");
				if ($self->want_feature('+migrated-v1-env')) {
					$self->add_files("overlay/addons/migration-db-override-names.yml");
				}
			}
		...
		# Other features
			...
			$self->add_files("cf-deployment/operations/use-compiled-releases.yml", "overlay/override-releases/compiled.yml");
			...
			$self->add_files("overlay/addons/nfs-volume-service.yml") unless $self->want_feature("bare");
			if ($self->want_feature("nfs-ldap") || $self->want_feature("nfs-ldap-tls")) {
				$self->add_files("cf-deployment/operations/enable-nfs-ldap.yml", "overlay/addons/nfs-ldap.yml");
				if ($self->want_feature("ocfp")) { $self->add_files("overlay/addons/nfs-ldap-config.yml"); }
				if ($self->want_feature("nfs-ldap-tls")) {
            ...
            bail("Failed to create operations/dynamic directory: " . join(", ", map { $_->{message} } @$err))
              if @$err;
            open my $fh_rem, '>', $remove_ops_file or bail("Cannot write to $remove_ops_file: $!");
			...
			$self->add_files("overlay/addons/smb-volume-service.yml") unless $self->want_feature("bare");
			...
			$self->add_files("overlay/enable-service-discovery.yml") unless $self->want_feature("bare");
		} elsif ($feature eq "trust-blacksmith-ca") {
			$self->add_files("overlay/addons/trust-blacksmith-ca.yml");
			if ($self->want_feature("cflinuxfs3")) { $self->add_files("overlay/addons/trust-blacksmith-ca-cflinuxfs3.yml"); }
			...
			$self->add_files("overlay/addons/stratos.yml"); # ocfp specific part handled in ocfp block
			...
			$self->add_files("overlay/addons/uaa-admin-client.yml"); # ocfp adds this too if not present
		...
		elsif (-f $self->env->path($self->{custom_ops_dir}."/${feature}.yml") || -f $self->env->path("ops/${feature}.yml")) {
			my $ops_file_path = -f $self->env->path($self->{custom_ops_dir}."/${feature}.yml")
    ...
    unless ($self->want_feature("internal-blobstore")) {
      ...
      "ocfp/$self->{iaas_name}/azs.yml",
		...
		if ($self->want_feature("stratos-integration")) { $self->add_files("ocfp/stratos.yml"); }
		...
		if ($self->want_feature("smb-volume-services")) { $self->add_files("ocfp/smb-broker.yml"); }
		if ($self->want_feature("trust-blacksmith-ca")) { $self->add_files("ocfp/trust-blacksmith-ca.yml");}

		# Add custom ops files collected for OCFP
		$self->add_files(@{$self->{opsfiles_for_ocfp}}) if @{$self->{opsfiles_for_ocfp}};
	}

	# --- Final Validations and non-bare specific additions ---
	if (scalar(@{$self->{blobstore_selections}}) > 1) {
		bail("Too many blobstores selected; pick only one of: " . join(", ", @{$self->{blobstore_selections}}));
	}
	if (scalar(@{$self->{database_selections}}) > 1) {
		bail("Too many databases selected; pick only one of: " . join(", ", @{$self->{database_selections}}));
	}

	my $has_availability_zones = exists($params_ref->{availability_zones});
	my $randomize_az_placement = $params_ref->{randomize_az_placement} ? lc($params_ref->{randomize_az_placement}) : 'false'; # lc for true/false
	if (($has_availability_zones || $randomize_az_placement eq 'true') && $self->want_feature("bare")) {
		bail("#M{params.availability_zones} and #M{params.randomize_az_placement}\n\tare not compatible with feature '#C{bare}'.");
	}
	...
	if (!$self->want_feature("bare")) {
		...
		my $skip_ssl_validation = $params_ref->{skip_ssl_validation} ? lc($params_ref->{skip_ssl_validation}) : '';
		if ($skip_ssl_validation eq 'false') {
			# Ensure this is added if not already by another feature
			unless ($self->has_file("cf-deployment/operations/stop-skipping-tls-validation.yml")) { # Assuming has_file check
			    $self->add_files("cf-deployment/operations/stop-skipping-tls-validation.yml"); # Added .yml
			}
		}
		if (scalar(@{$self->{database_selections}}) == 0) { # If still no DB after defaults in pre-val
