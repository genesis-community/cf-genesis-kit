...
sub init {
    ...
    $obj->{iaas_name} =
      $obj->env->iaas;    # aws, gcp, azure, vsphere, openstack, stackit
...
"- #y{Experimental Feature Enabled:} Custom cf-deployment version: $version"
    ...
    my $cfd_file =
      "$genesis_root/.genesis/kits/addons/cf-deployment-${version}.tar.gz";
    my $cfd_url =
"https://github.com/cloudfoundry/cf-deployment/archive/v${version}.tar.gz";
...
"  #i{Fetching cf-deployment-${version} release from cloudfoundry/cf-deployment}",
        ...
        eval {
            ...
            my ( $curl_out, $curl_rc ) = run( { stderr => 0 },
                'curl', '-s', '-o', $tmp_file, '-L', $cfd_url );
        ...
        if ( $tar_rc != 0 ) {
            bail(
"Failed to list contents of cf-deployment v${version} tarball: $tar_output"
            );
        }
...
sub _dynamic_isolation_segments {
    ...
    {
        push @iso_seg_merges,
          "overlay/dynamic-templates/isolation-segment-network.yml";
    }
    if ( $self->want_feature("cflinuxfs3") ) {
        push @iso_seg_merges,
          "overlay/dynamic-templates/isolation-segment-cflinuxfs3.yml";
    }
    if ( $self->want_feature("nfs-volume-services") ) {
        push @iso_seg_merges,
          "overlay/dynamic-templates/isolation-segment-nfs.yml";
        if (   $self->want_feature("nfs-ldap")
            || $self->want_feature("nfs-ldap-tls") )
        {
            push @iso_seg_merges,
              "overlay/dynamic-templates/isolation-segment-nfs-ldap.yml";
...
"overlay/dynamic-templates/isolation-segment-nfs-ldap-ocfp.yml",
    ...
    if ( $self->want_feature("smb-volume-services") ) {
        push @iso_seg_merges,
          "overlay/dynamic-templates/isolation-segment-smb.yml";
    }
    if ( $self->want_feature("ocfp") ) {
    ...
    foreach my $group (@isolation_groups) {
        ...
        my $isolation_segments =
          decode_json($params_json_str)->{isolation_segments};
        ...
        if ( $self->want_feature("ocfp") || $has_additional_trusted_certs ) {
            push @additional_trusted_certs_files,
              $self->_dynamic_isolation_template_render(
                "additional-trusted-certs", $group );
            if ( $self->want_feature("cflinuxfs3") ) {
                push @additional_trusted_certs_files,
                  $self->_dynamic_isolation_template_render(
                    "additional-trusted-certs-cflinuxfs3", $group );
            }
            if ( $self->want_feature("ocfp") ) {
                push @additional_trusted_certs_files,
                  $self->_dynamic_isolation_template_render(
                    "ocfp-trusted-certs", $group );
            }
        }
        $additional_trusted_certs_str =
          join( " ", @additional_trusted_certs_files );

        my $dynamic_segment_fragment_file =
          "overlay/dynamic/isolation-segments-$group.yml";
        ...
        my $append_json =
          '{"instance_groups": [ "((prepend))", "((defer append))" ]}';
        ...
        if ( $spruce_rc != 0 ) {
            bail(
"Failed to merge isolation segment '$group' with spruce: $spruce_err"
            );
        }
        ...
        {
            $self->_dynamic_isolation_template_render( "nfs-ldap-config",
                $group );
        }
...
sub _dynamic_instance_vm_types {
    ...
    my $types_op_file_content =
      "--- # Dynamically created instance type overrides\n";
    my $found_vm_types = 0;
    ...
    foreach my $key ( keys %$params_ref ) {
        if ( $key =~ /^(.*)_vm_type$/ ) {
            ...
            if    ( $inst_grp eq 'errand' || $inst_grp eq 'haproxy' ) { next; }
                ...
                $inst_grp = "diego_cell";
                warning(
"Translated: params.cell_vm_type => params.diego_cell_vm_type"
                );
            }
            elsif ( $inst_grp eq 'diego' ) {
                $inst_grp = "scheduler";
                warning(
"Translated: params.diego_vm_type => params.scheduler_vm_type"
                );
            }
            elsif ( $inst_grp eq 'bbs' ) {
                $inst_grp = "diego_api";
                warning(
                    "Translated: params.bbs_vm_type => params.diego_api_vm_type"
                );
            }
            elsif ( $inst_grp eq 'loggregator' ) {
                $inst_grp = "log_api";
                warning(
"Translated: params.loggregator_vm_type => params.log_api_vm_type"
                );
            }
            elsif ( $inst_grp eq 'postgres' ) {
                $inst_grp = "database";
                warning(
"Translated: params.postgres_vm_type => params.database_vm_type"
                );
            }
...
"Unknown instance group $dashed_inst_grp (from $inst_grp_orig) - this may be bug in your environment files."
            ...
            $types_op_file_content .= $self->_gopatch_replace(
                "/instance_groups/name=$dashed_inst_grp/vm_type", $type );
    ...
    if ($errand_vm_type) {
        ...
        foreach my $errand_name (qw(smoke-tests rotate-cc-database-key)) {
            if ( $used_groups_for_vm_types !~ /^$errand_name$/m ) {
                $types_op_file_content .= $self->_gopatch_replace(
                    "/instance_groups/name=$errand_name/vm_type",
                    $errand_vm_type );
    ...
    if ($found_vm_types) {
        ...
        if (@dups) {
            bail(
"Instance vm types specified (or translated as) multiple times: "
                  . join( ", ", @dups ) );
        }
    ...
    foreach my $key ( keys %$params_ref ) {
        if ( $key =~ /^(.*)_instances$/ ) {
            ...
            if ( $inst_grp eq 'errand' || $inst_grp eq 'haproxy' ) {
                ...
                $inst_grp = "diego_cell";
                warning(
"Translated: params.cell_instances => params.diego_cell_instances"
                );
            }
            elsif ( $inst_grp eq 'diego' ) {
                $inst_grp = "scheduler";
                warning(
"Translated: params.diego_instances => params.scheduler_instances"
                );
            }
            elsif ( $inst_grp eq 'bbs' ) {
                $inst_grp = "diego_api";
                warning(
"Translated: params.bbs_instances => params.diego_api_instances"
                );
            }
            elsif ( $inst_grp eq 'loggregator' ) {
                $inst_grp = "log_api";
                warning(
"Translated: params.loggregator_instances => params.log_api_instances"
                );
            }
            elsif ( $inst_grp eq 'postgres' ) {
                $inst_grp = "database";
                warning(
"Translated: params.postgres_instances => params.database_instances"
                );
            }
            elsif ( $inst_grp eq 'blobstore' ) {
                $inst_grp = "singleton-blobstore";
...
"Unknown instance group $dashed_inst_grp (from $inst_grp_orig) - this may be bug in your environment files."
            ...
            $counts_opsfile_content .= $self->_gopatch_replace(
                "/instance_groups/name=$dashed_inst_grp?/instances", $count );
    ...
    if ($errand_instances) {
        ...
        foreach my $errand_name (qw(smoke-tests rotate-cc-database-key)) {
            if ( $used_groups_for_counts !~ /^$errand_name$/m ) {
                $counts_opsfile_content .= $self->_gopatch_replace(
                    "/instance_groups/name=$errand_name?/instances",
                    $errand_instances );
    ...
    if ($found_counts) {
        ...
        if (@dups) {
            bail(
                "Instance counts specified (or translated as) multiple times: "
                  . join( ", ", @dups ) );
        }
...
sub _perform_feature_pre_validation {
    ...
    for my $want ( @{ $self->{raw_features} } ) {
        ...
        if ( $want =~ /^cf-deployment-version-(.*)$/ ) {
            ...
            warning(
"The #c{$want} feature has been deprecated, in favor of BOSH add-ons"
            );
        }
        elsif ( $want =~
...
"The #c{$want} feature is now the default behaviour and doesn't need\n\tto be specified in the environment file"
            ...
            push @curated_features,
              "+internal-blobstore"
              unless $self->want_feature_in_list( "+internal-blobstore",
                \@curated_features );
        }
        elsif ( $want =~ /^blobstore-(aws|azure|gcp)$/ ) {
            my $iaas = $1;
            warning(
                "The #c{$want} feature has been renamed to #c{$iaas-blobstore}"
            );
            ...
            /^(internal-db|db-internal-postgres|local-db|\+internal-db)$/ )
        {
            push @curated_features, "+internal-db"
              unless $self->want_feature_in_list( "+internal-db",
                \@curated_features );
            warning(
                "The #c{$want} flag has been renamed to #c{local-postgres-db}");
...
"The #c{haproxy-notls} feature flag has been deprecated.\n\tPlease replace it with the #c{haproxy} feature flag.\n\tYou are HIGHLY ENCOURAGED to also add the #c{tls} flag."
            ...
            warning(
"The 'minimum-vms' feature flag has been renamed to 'small-footprint'"
            );
            push @curated_features, "small-footprint";
        }
        elsif ( $want eq "azure" && $self->{iaas_name} eq "azure" )
        {    # only warn if it's redundant
            warning(
"The #c{azure} feature does not have to be specified, as it will automatically be applied when deploying via an Azure CPI"
            ...
            bail(
                "The #c{no-nats-tls} feature is no longer able to be supported."
            );
        }
...
"The #c{$want} feature is no longer supported; it has been replaced by the\n\tupstream #c{cf-deployment/operations/enable-service-discovery} feature."
            ...
            push @curated_features,
              "enable-service-discovery";    # this will be handled below
        }
            ...
            push @curated_features,
              "compiled-releases"
              unless $self->want_feature_in_list(
                "cf-deployment/operations/use-compiled-releases",
                \@curated_features );
        }
        elsif ( $want =~
            /^(small-footprint|cf-deployment\/operations\/scale-to-one-az)$/ )
        {
            push @curated_features,
              "small-footprint"
              unless $self->want_feature_in_list( "small-footprint",
                \@curated_features );
        }
        elsif ( $want =~
/^(nfs-volume-services|cf-deployment\/operations\/enable-nfs-volume-service)$/
        ...
        {    # Corrected regex
            push @curated_features,
              "nfs-volume-services"
              unless $self->want_feature_in_list( "nfs-volume-services",
                \@curated_features );
        }
        elsif ( $want =~
/^(smb-volume-services|cf-deployment\/operations\/enable-smb-volume-service)$/
        ...
        {    # Corrected regex
            push @curated_features,
              "smb-volume-services"
              unless $self->want_feature_in_list( "smb-volume-services",
                \@curated_features );
        }
        elsif ( $want =~
/^(nfs-ldap|nfs-ldap-tls|cf-deployment\/operations\/enable-nfs-ldap)$/
          )
        {    # Corrected regex
            if (
                !$self->want_feature_in_list( 'nfs-volume-services',
                    \@curated_features )
...
# 'enable-service-discovery' is special, ensure it's added if not bare (handled above via alias)
                ...
                && $self->want_feature_in_list( "bare", \@curated_features ) )
            {
                ...
                push @curated_features, $want
                  unless $self->want_feature_in_list( $want,
                    \@curated_features );
            }
        }
        elsif ( $want =~
...
"#c{$want} was not found in upstream files.\n\tSee cf-deployment for valid ops files."
            ...
            push @curated_features,
              ...
              unless $self->want_feature_in_list( "enable-service-discovery",
                \@curated_features )
              || $self->want_feature_in_list( "bare", \@curated_features );
            push @curated_features,
              "uaa-admin-client"
              unless $self->want_feature_in_list( "uaa-admin-client",
                \@curated_features );
            push @curated_features, $want
              unless $self->want_feature_in_list( $want, \@curated_features );
        }
                ...
                || -f "$env_root/ops/$want.yml" )
            {
                ...
                bail(
"The #c{$want} feature is not supported, see MANUAL.md for valid features."
                );
            }
        }
    }
...
# Handle OCFP blobstore selection (needs curated_features for want_feature("ocfp"))
    if ( $self->want_feature_in_list( "ocfp", \@curated_features ) ) {
        ...
        if (
            $blobstore_feature_for_ocfp
            && !$self->want_feature_in_list(
                $blobstore_feature_for_ocfp, \@curated_features
            )
          )
        {
...
/^(minio-blobstore|aws-blobstore|aws-blobstore-iam|azure-blobstore|gcp-blobstore)$/
    ...
    if ( $self->{abort_flag} ) {
        bail(
"#R{Cannot continue} - fix the #C{$ENV{GENESIS_ENVIRONMENT}.yml} file."
        );
    }
    ...
    foreach my $want ( @{ $self->{raw_features} } ) {
        if ( $want =~ /^cf-deployment-version-(.*)$/ ) {
            if ($custom_cf_version) {
                bail(
"You cannot specify more than one cf-deployment-version-* feature"
                );
            }
            $custom_cf_version = $1;
        }
    ...
    $self->_perform_feature_pre_validation()
      ;    # Populates $self->{processed_features}
...
# This ensures the base cf-d scale-to-one-az is present if azure or explicitly requested.
            ...
            $self->add_files("operations/scale-to-one-az.yml")
              ;    # Our overlay for scale-to-one-az
        }
...
"operations/dynamic/remove-unused-nfs-ldap-ca-cert.yml";
                        ...
                        print $fh_rem $self->_gopatch_remove(
                            "/variables/name=nfs-ldap-ca-cert");
                        close $fh_rem;
            ...
            if ( $self->want_feature("cflinuxfs3") ) {
                $self->add_files(
                    "overlay/addons/trust-blacksmith-ca-cflinuxfs3.yml");
            }
...
"cf-deployment/operations/experimental/use-compiled-releases-windows.yml",
        ...
        elsif ( $feature =~ /^cf-deployment\// && -f "$feature.yml" )
        {          # Upstream ops files
            ...
            || -f $self->env->path("ops/${feature}.yml") )
        {
            ...
            if ( $self->want_feature("ocfp") ) {
                push @{ $self->{opsfiles_for_ocfp} },
                  $ops_file_path;    # Collect for OCFP block
            }
            else {
...
# NOTE: isolation-segments and ocfp are major features often with their own blocks
    ...
    if ( $self->want_feature("isolation-segments") ) {
        ...
        my @segment_files =
          $self->_dynamic_isolation_segments();    # Uses $self->{parsed_params}
    ...
    if ( $self->want_feature("ocfp") ) {
        ...
        unless (
            $self->want_feature("local-postgres-db|internal-db|\+internal-db") )
        {
            if ( $self->{iaas_name} =~ /^(aws|azure|gcp)$/ ) {
                $self->add_files( "ocfp/external-db-prep.yml",
        ...
        if ( $self->want_feature("windows-diego-cells") ) {
            $self->add_files(
                "ocfp/$self->{iaas_name}/windows.yml",
        ...
        if ( $self->want_feature("nfs-volume-services") )
        {    # Assumes nfs-ldap implied for ocfp nfs
...
"#M{params.availability_zones} and #M{params.randomize_az_placement}\n\tare not compatible with feature '#C{bare}'."
    ...
    if ( !$self->want_feature("bare") ) {
        ...
        {
            $self->add_files(
                "overlay/addons/migration-v1-nats-credentials.yml");
        }
        ...
        if ( $self->{iaas_name} eq 'azure' ) {
            if (
                (
                    ...
                    || $randomize_az_placement eq 'true'
                )
              )
...
"#M{params.availability_zones} and #M{params.randomize_az_placement} are\n\tnot compatible with deployments to Azure infrastructure."
    ...
    my $exodus_version =
      $self->env->exodus_lookup( "kit_version", "" );    # Use $self->env
