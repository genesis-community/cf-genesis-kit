...
sub perform {
    ...
    my $system_domain =
      $exodus_data->{system_domain} || "system." . $exodus_data->{base_domain};
...
"No API domain found in exodus data. Please ensure the deployment has completed successfully."
    ...
    info( "Based on #M{cf-deployment %s}%s\n"
          ...
          . "  username: #M{%s}\n"
          . "  password: #G{%s}\n",
        $upstream_version, $hotfixes, $upstream_url, $api_url, $admin,
        $password );
    ...
    my $cf_curl_output =
      run( { onfailure => "Error executing 'cf curl /info'", stderr => 0 },
        'cf', 'curl', '/info' );
