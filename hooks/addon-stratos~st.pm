...
sub _get_stratos_info {
    ...
    my $deployment_name =
      $env->lookup( 'stratos.deployment_name', $self->env->name . "-stratos" );
    ...
    my $stratos_version = $self->{options}->{version}
      // $env->lookup( 'stratos.version', '4.8.1' );
    ...
    my $stratos_db_sslmode =
      $env->params->{db_sslmode} || 'disabled';    # verify-ca
    ...
    if ( $self->env->has_feature('ocfp') ) {
        ...
        $stratos_domain =
          $env->vault->get( $env->secrets_base . "fqdns:stratos" ) || '';
        $stratos_url = "https://${stratos_domain}";
        $stratos_db_scheme =
          $env->vault->get( $env->secrets_base . "stratos/db/stratos:scheme" )
          || 'postgres';
        $stratos_db_hostname =
        ...
        $stratos_db_password =
          ...
          || 'stratos';
        $stratos_db_port =
          $env->vault->get( $env->secrets_base . "stratos/db/stratos:port" )
          || 5432;
        $stratos_db_database =
    ...
    my $admin_password =
      $env->vault->get( $env->secrets_base . "stratos/admin_password" ) || "";
    my $session_secret =
    ...
    return {
        name =>
          $env->lookup( 'stratos.deployment_name', $env->name . "-stratos" ),
...
sub display_info {
    ...
    if ( $options{version} && $options{version} ne $info->{version} ) {
        ...
        info( "Note: Using Stratos version %s from command line",
            $options{version} );
    }
    ...
    if ( $info->{cf_api} ) {
        info(
            "\n"
              ...
              . "\n  Space: %s"
              . "\n  App Name: %s",
            $info->{cf_api},      $info->{system_domain},
            $info->{apps_domain}, $info->{cf_org},
            $info->{cf_space},    $info->{cf_app_name}
        );
    }
    ...
    if ( $info->{db}->{hostname} ) {
        info(
            "\nDatabase Configuration:\n"
              ...
              . "  Username: %s\n"
              . "  SSL Mode: %s",
            $info->{db}->{scheme},   $info->{db}->{hostname},
            $info->{db}->{port},     $info->{db}->{database},
            $info->{db}->{username}, $info->{db}->{sslmode}
        );
    }
    ...
    info("\nHelpful Commands:");
    info(
        "  Open in browser: %s %s stratos open",
        $self->env->get_call_path_with_env()
    );    # returns two strings
    info(
        "  Deploy Stratos: %s %s stratos deploy",
        $self->env->get_call_path_with_env()
    );    # returns two strings
...
"CF CLI not found. Please install it or use --skip-cf-check if you're sure it's available."
    ...
    unless ( $stratos_client && $stratos_client_secret ) {
        warning(
            "Stratos client credentials not found in vault. Using defaults.");
...
"https://github.com/cloudfoundry/stratos/releases/download/v${stratos_version}/stratos-ui-${stratos_version}.zip";
    my $stratos_sso_options =
      $env->lookup( 'stratos.sso_options', 'nosplash, logout' );
    ...
    if ( $options{file} && -f $options{file} ) {
        ...
        run( { onfailure => "Failed to unzip Stratos file" },
            'unzip', '-o', $options{file} );
    }
    else {
        info( "Downloading Stratos %s...", $stratos_version );
        run( { onfailure => "Failed to download Stratos" },
    ...
    my ( $svc_list, $svc_rc ) = run( { stderr => 0 }, 'cf', 'curl',
"/v3/service_instances?organization_guids=${org_guid}&space_guids=${space_guid}"
    ...
    if ( $jq_rc == 0 ) {
        ...
        ( $svc_exists, $jq_rc ) = run(
            { stderr => 0 },
            'jq', '-r',
            ".resources[]|select(.name|test(\"${svc_name}\"))|.name",
            $temp_svc_file
        );
...
EOF
    ...
    if ( $svc_exists eq $svc_name ) {
        info(
            "Service %s was found, updating existing cups service definition.",
            $svc_name
        );
        ...
        info( "Service %s was not found, creating cups service definition.",
            $svc_name );
...
sub open_in_browser {
    ...
    my $cmd =
      $^O eq 'darwin' ? 'open' : ( $^O eq 'MSWin32' ? 'start' : 'xdg-open' );
