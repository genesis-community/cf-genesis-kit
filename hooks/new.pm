...
sub ask_for_database {
	...
	$self->{database} = prompt_for( "select",
		'Where would you like to house Cloud Foundry configuration and metadata?', @options );
...
sub perform {
	...
	while ( $ok ne 'true' ) {
		...
		$self->{base_domain} =
		  prompt_for( "line", "What is the base domain of your Cloud Foundry?" );
...
"What is the apps domain of your Cloud Foundry? (press enter for default: run.{base_domain})",
	...
	if ( $use_bare eq 'true' ) {
		...
		my $network_topography = prompt_for(
			...
			-o =>
			  "[partitioned-network] Partitioned network that separates core, edge and runtime vms",
		...
		if ( $iaas eq 'azure' ) {
			...
			info("#gu{Blobstore}");
			my $use_azure_storage = prompt_for( "boolean",
			...
			if ( $use_azure_storage eq 'true' ) {
				...
				prompt_for( "$ENV{GENESIS_SECRETS_BASE}blobstore:storage_account_name",
					"secret-line", 'What is your Azure Storage Account Name?' );

				prompt_for( "$ENV{GENESIS_SECRETS_BASE}blobstore:storage_access_key",
					"secret-line", 'What is your Azure Storage Account Key?' );

				$self->move_secrets_to_credhub( "blobstore:storage_account_name",
					"blobstore_storage_account_name" );

				$self->move_secrets_to_credhub( "blobstore:storage_access_key",
			...
			my $use_aws_stuff = prompt_for( "boolean",
			...
			if ( $use_aws_stuff eq 'true' ) {
				...
				$self->{aws_blobstore_region} =
				  prompt_for( "line", 'What region contains the your Amazon S3 blobstore?' );

				$self->move_secrets_to_credhub( "blobstore:aws_access_key",
					"blobstore_access_key_id" );

				$self->move_secrets_to_credhub( "blobstore:aws_access_secret",
				...
				$self->{bucket_prefix} =
				  ( $env_name eq lc( $ENV{GENESIS_ENVIRONMENT} ) ) ? '' : $env_name;
			}
		}
			...
			my $use_gcp_blobstore = prompt_for(
				...
				-o =>
				  '[gcpaccess] Existing Google Cloud Storage accessed via access and secret keys',
				-o =>
				  '[builtin] Local singleton blobstore that will be deployed in this deployment',
				--default => 'gcp'
			);
...
'What is the Cloud Storage Service Account ID (@<project>.iam.gserviceaccount.com)?'
				...
				$self->move_secrets_to_credhub( "blobstore:gcp_project_name", "gcs_project" );

				$self->move_secrets_to_credhub( "blobstore:gcp_client_email",
					"gcs_service_account_email" );

				$self->move_secrets_to_credhub( "blobstore:gcp_json_key",
				...
				$self->move_secrets_to_credhub( "blobstore:gcp_access_key",
					"blobstore_access_key_id" );

				$self->move_secrets_to_credhub( "blobstore:gcp_secret_key",
					"blobstore_secret_access_key" );
			}
		}
		...
		my $use_autoscaler = prompt_for( "boolean",
		...
		my $use_prometheus = prompt_for( "boolean",
